/**
 * Re-Migration Script: Case Studies to Sanity with FULL HTML formatting
 * 
 * This script re-migrates existing case studies, properly converting
 * WordPress HTML to Sanity Portable Text with all formatting preserved.
 * 
 * Usage: node scripts/remigrate-casestudies-with-formatting.js
 */
...
async function updateCaseStudy(study, index) {
  const slug = study.slug;
  
  try {
    // Find existing case study by slug
    ...
    if (!existing) {
      console.log(`  [${index + 1}/${caseStudies.length}] ⏭ Not found: ${study.title.substring(0, 40)}...`);
      return { notFound: true };
    }

    // Convert HTML to Portable Text
    const portableTextContent = htmlToPortableText(study.content);
    
    if (portableTextContent.length === 0) {
      console.log(`  [${index + 1}/${caseStudies.length}] ⚠ No content: ${study.title.substring(0, 40)}...`);
      return { noContent: true };
    }

    // Update case study with new content
    await client.patch(existing._id)
      .set({ content: portableTextContent })
      .commit();

    console.log(`  [${index + 1}/${caseStudies.length}] ✓ Updated: ${study.title.substring(0, 40)}... (${portableTextContent.length} blocks)`);
    return { updated: true };
  } catch (error) {
...
async function migrate() {
  ...
  console.log('Case Studies Re-Migration with Full Formatting');
  console.log('='.repeat(60));
  ...
  console.log('\nUpdating case studies with formatted content...');

  for (let i = 0; i < caseStudies.length; i++) {
    const result = await updateCaseStudy(caseStudies[i], i);
