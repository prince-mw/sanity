...
const ProductCard = ({ icon, label, href, index }: { icon: React.ReactNode; label: string; href: string; index: number }) => {
  ...
  const prefix = parts[0]; // "MW"
  const productName = parts.slice(1).join(' '); // "Planner", "Measure", etc.
...
export default function PlatformPageClient() {
  ...
  const products = [
    ...
    { icon: <InventoryIcon />, label: "MW Inventory", href: "/mw-inventory" },
...
const APIHubIcon = ({ className }: { className?: string }) => (
  <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const InventoryIcon = () => (
  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7l9-4 9 4-9 4-9-4zm0 0v10l9 4 9-4V7M3 7l9 4m0 0l9-4m-9 4v10" />
  </svg>
);
