...
export interface DocumentationProps {
  title?: string;
  subtitle?: string;
  endpoints?: { method: string; endpoint: string; description: string; params?: string[] }[];
  sdks?: { name: string; language: string; installCommand: string; docsUrl?: string }[];
  ctaTitle?: string;
}
...
const defaultContent: DocumentationProps = {
...
export default function DocumentationClient({ content }: { content?: DocumentationProps }) {
  const [activeSidebar, setActiveSidebar] = useState("quickstart");

  const pageTitle = content?.title || defaultContent.title;
  const pageSubtitle = content?.subtitle || defaultContent.subtitle;
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-mw-gray-900 mb-6">
              {pageTitle?.includes("&") ? (
                <>
                  ...
                  <span className="text-mw-blue-600 block">{pageTitle.split("&")[1].trim()}</span>
                </>
              ) : (
                <>
                  Documentation &
              ...
              <div className="bg-white border border-mw-gray-200 rounded-xl p-8">
                ...
                <div className="prose max-w-none">
                  ...
                  {content?.endpoints && content.endpoints.length > 0 && (
                    <>
                      <h3 className="text-2xl font-bold text-mw-gray-900 mt-12 mb-4">API Endpoints</h3>
                      <div className="space-y-4 mb-8">
                        {content.endpoints.map((ep, idx) => (
                          <div key={idx} className="bg-mw-gray-50 rounded-lg p-4 border border-mw-gray-200">
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <code className="text-sm text-mw-gray-800 font-mono">{ep.endpoint}</code>
                            </div>
                            <p className="text-mw-gray-600 text-sm">{ep.description}</p>
                            {ep.params && ep.params.length > 0 && (
                              <div className="mt-2 flex flex-wrap gap-2">
                                {ep.params.map((param, pIdx) => (
                                  <span key={pIdx} className="text-xs bg-mw-gray-200 text-mw-gray-700 px-2 py-1 rounded font-mono">{param}</span>
                                ))}
                              </div>
                            )}
                          </div>
                  ...
                  {/* SDKs from CMS */}
                  {content?.sdks && content.sdks.length > 0 && (
                    <>
                      <h3 className="text-2xl font-bold text-mw-gray-900 mt-12 mb-4">SDKs & Libraries</h3>
                      <div className="grid md:grid-cols-2 gap-4 mb-8">
                        {content.sdks.map((sdk, idx) => (
                          <div key={idx} className="bg-mw-gray-50 rounded-lg p-4 border border-mw-gray-200">
                            <h4 className="font-bold text-mw-gray-900 mb-1">{sdk.name}</h4>
                            <p className="text-sm text-mw-gray-500 mb-2">{sdk.language}</p>
                            <code className="text-xs bg-mw-gray-200 text-mw-gray-700 px-2 py-1 rounded block font-mono">{sdk.installCommand}</code>
                            {sdk.docsUrl && (
                              <a href={sdk.docsUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-mw-blue-600 hover:underline mt-2 inline-block">View Docs →</a>
                            )}
                          </div>
                        ))}
                      </div>
