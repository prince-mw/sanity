...
export default function PressNewsPage() {
  const [pressReleases, setPressReleases] = useState(staticPressReleases);
  const [mediaFeatures, setMediaFeatures] = useState(staticMediaFeatures);
    ...
    async function fetchData() {
      try {
        ...
        if (sanityPressReleases && sanityPressReleases.length > 0) {
          setPressReleases(sanityPressReleases.map(transformPressRelease));
        }
        
        if (sanityMediaFeatures && sanityMediaFeatures.length > 0) {
          setMediaFeatures(sanityMediaFeatures.map(transformMediaFeature));
        }
      } catch (error) {
