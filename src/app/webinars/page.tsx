...
import { getUpcomingWebinars, getOnDemandWebinars, transformWebinar, SanityWebinar } from "../../sanity/lib/fetch";

// Static fallback data
const staticUpcomingWebinars = [
  {
...
export default function WebinarsPage() {
  ...
  const [upcomingWebinars, setUpcomingWebinars] = useState(staticUpcomingWebinars);
  const [onDemandWebinars, setOnDemandWebinars] = useState(staticOnDemandWebinars);
    ...
    async function fetchWebinars() {
      try {
        ...
        if (upcoming && upcoming.length > 0) {
          setUpcomingWebinars(upcoming.map(transformWebinar));
        }
        if (onDemand && onDemand.length > 0) {
          setOnDemandWebinars(onDemand.map(transformWebinar));
        }
      } catch (error) {
